...
FPos UPlayGame::AI_CalculatePos() {
	FPos Result = MinMax(FPos(-1, -1), 0, 4, BoardStates, !Turn, INT64_MIN, INT64_MAX).Key;
	return Result;
}

TTuple<FPos, int64> UPlayGame::MinMax(FPos ClickedPos, int64 CurrDepth, int64 MaxDepth,TMap<FPos, EBoardState> Table, bool CPUTurn, int64 Alpha, int64 Beta) {
	...
	if (CPUTurn) {
		...
		for (int i = 0; i < SetUpPos.Num(); i++) {
			...
			TTuple<FPos, int64> TempValue = MinMax(SetUpPos[i], CurrDepth + 1, MaxDepth, NextTable, !CPUTurn, Alpha, Beta);
